...
if __name__ == "__main__":
    ...
    for i_iter in range(args.n_iter):
        ...
        if i_iter % args.eval_step == 0:
            ...
            torch.save(saved_weights, ensure_dir(path_join("data", args.data_dir, model.name, "{}_saved_eval_iter-{}_temp-{}_rec.bin".format(args.dataset_prefix, int(i_iter/args.eval_step), args.temp))))
            ...
            if best_model > iter_eval:
                ...
                torch.save(model, path_join("data", args.data_dir, "{}_hidden-{}_drop-{}_rec.pt".format(model.name, args.hidden_dim, args.drop_prob)))

    # test performance
    model = torch.load(path_join("data", args.data_dir, "{}_hidden-{}_drop-{}_rec.pt".format(model.name, args.hidden_dim, args.drop_prob)))
    ...
    print("{}_{} - test RMSE: {}".format(model.name, args.hidden_dim, iter_test))
    torch.save(saved_weights, ensure_dir(path_join("data", args.data_dir, model.name, "{}_saved_test_adam_temp-{}_rec.bin".format(args.dataset_prefix, args.temp))))
